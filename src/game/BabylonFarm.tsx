...
import { Engine, HemisphericLight, MeshBuilder, Scene, StandardMaterial, Color3, Vector3, UniversalCamera, DirectionalLight, TransformNode, PhotoDome, Mesh, SceneLoader } from 'babylonjs'
  ...
  useEffect(() => {
    ...
    cropPositions.forEach((crop) => {
      ...
      mat.diffuseColor = Color3.FromHexString(
        crop.severity === 3 ? '#b56b74' : crop.severity === 2 ? '#7f9f8b' : '#6e8d98'
      )
      stalk.material = mat
    })
    ...
    const animalModels = [
      ...
      { id: 'chicken-2', file: 'chicken.glb', position: new Vector3(1.2, 0, 1.2), scale: 0.9, rotationY: -Math.PI * 0.2 },
      { id: 'chicken-3', file: 'chicken.glb', position: new Vector3(-1.2, 0, 1.4), scale: 0.9, rotationY: Math.PI * 0.9 },
    ]
